    ...
    <Section class="!pb-8 !pt-16 md:!pt-24 text-center">
      <Container class="max-w-2xl">
        ...
        <Heading :level="1" class="mb-6"
          >Gather with <span class="text-brand-violet">us</span>.</Heading
        >
        <Body large class="mb-12">
          From virtual demo days to in-person symposiums, our events bring together the brightest
          young minds and the mentors who support them.
        </Body>

        <div
          class="flex justify-center gap-2 bg-surface p-1.5 rounded-full border hairline-ink shadow-sm w-fit mx-auto"
        >
          <button
            type="button"
      ...
      <Container class="max-w-4xl">
        ...
        >
          ...
          <template v-else-if="showEmpty">
            <div
              class="py-24 text-center flex flex-col items-center justify-center max-w-lg mx-auto"
            >
              <div
                class="w-32 h-32 bg-ink/5 rounded-full flex items-center justify-center mb-6 text-ink/20 relative"
              >
              ...
              <Heading :level="3" class="mb-2"
                >No {{ tab === 'upcoming' ? 'upcoming' : 'past' }} events.</Heading
              >
              <Body class="mb-8">
          ...
          <template v-else>
            ...
            >
              ...
              >
                <UiCard
                  class="flex flex-col md:flex-row items-start md:items-center gap-6 p-6 md:p-8 hover:shadow-md transition-shadow bg-surface"
                >
                  <div
                    class="bg-ink/5 rounded-xl px-5 py-4 text-center min-w-[80px] shrink-0 border hairline-ink group-hover:bg-brand-violet/5 group-hover:!border-brand-violet/20 transition-colors"
                  >
                    <div
                      class="text-sm font-semibold text-ink/60 uppercase tracking-widest group-hover:text-brand-violet transition-colors"
                    >
                      {{ formatDate(event.datetime).month }}
                    </div>
                    <div class="font-display font-bold text-3xl text-ink">
                  ...
                  <div class="flex flex-col gap-2.5 flex-1">
                    <div class="flex items-center gap-3 mb-1">
                      <UiChip
                        class="group-hover:bg-gradient-brand group-hover:!text-white group-hover:!border-transparent transition-all"
                        >{{ event.type }}</UiChip
                      >
                      <span
                        v-if="event.isVirtual"
                        class="text-xs font-semibold text-ink/50 uppercase tracking-wider flex items-center gap-1"
                      >
                    ...
                    <Heading
                      :level="3"
                      class="!text-2xl group-hover:text-brand-violet transition-colors"
                    >
                      {{ event.title }}
                    </Heading>
                    <div class="flex items-center gap-2 text-sm text-ink/70 font-medium">
                      <Icon icon="lucide:map-pin" width="16" class="text-ink/40" />
                      {{ event.location }}
                    </div>
                  </div>

                  <div
                    class="mt-4 md:mt-0 md:ml-auto w-full md:w-auto flex items-center justify-end"
                  >
                    <UiButton
