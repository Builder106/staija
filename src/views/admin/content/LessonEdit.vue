...
async function load() {
  ...
  try {
    ...
    form.value = {
      ...
      completionCriteria:
        (f.completionCriteria as 'viewed' | 'assignment_submitted' | 'quiz_passed') ?? 'viewed',
      quiz: quizId,
    };
    markClean();
  } catch (err) {
...
async function copyText(text: string, key: string) {
  try {
    ...
    setTimeout(() => {
      if (aiCopiedKey.value === key) aiCopiedKey.value = null;
    }, 1500);
  } catch {
            ...
            <div class="grid md:grid-cols-2 gap-4">
              <div class="flex flex-col gap-2">
                ...
                <p
                  ...
                  class="text-[11px] text-emerald-700 inline-flex items-center gap-1"
                >
                  <Icon icon="lucide:check-circle-2" width="11" /> YouTube link — embeds inline in
                  the lesson player.
                </p>
                <p
                  ...
                  class="text-[11px] text-emerald-700 inline-flex items-center gap-1"
                >
                  <Icon icon="lucide:check-circle-2" width="11" /> Vimeo link — embeds inline in the
                  lesson player.
                </p>
                <p
                  v-else-if="videoProvider === 'other'"
                  class="text-[11px] text-amber-700 inline-flex items-center gap-1"
                >
                  <Icon icon="lucide:alert-triangle" width="11" /> Not a YouTube or Vimeo URL — the
                  player loads it raw, which only works if the host allows iframe embedding.
                </p>
                <p
                  v-else-if="videoProvider === 'invalid'"
                  class="text-[11px] text-red-700 inline-flex items-center gap-1"
                >
                  <Icon icon="lucide:x-circle" width="11" /> Doesn't look like a URL.
                </p>
                <p v-else class="text-[11px] text-ink/50">
                  Paste a YouTube or Vimeo watch-page link. The lesson player auto-converts it to an
                  embed. Leave blank for a text-only lesson.
                </p>
              </div>
              <div class="flex flex-col gap-2">
                <label
                  for="lesson-completion-criteria"
              ...
              <div
                v-if="form.completionCriteria === 'quiz_passed'"
                class="flex flex-col gap-2 pt-2 border-t border-ink/10"
              >
                <label class="text-xs font-semibold text-ink/70 uppercase tracking-wide"
                  >Linked Quiz</label
                >
          ...
          <UiCard class="p-6 md:p-8 bg-surface">
            ...
            <div v-if="aiOpen" class="mt-5 flex flex-col gap-5">
              <p class="text-xs text-ink/60 m-0 leading-relaxed">
                Suggests YouTube searches, image searches, a 90-second narration script you can
                record, and key-concept bullets — based on the title and body above. Editors curate;
                nothing is inserted automatically.
              </p>

              <div class="flex items-center gap-3 flex-wrap">
                <UiButton variant="primary" :disabled="!canAssist || aiLoading" @click="runAssist">
                  <Icon v-if="aiLoading" icon="lucide:loader-2" width="14" class="animate-spin" />
                  ...
                  <div class="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      >
                        <Icon
                          :icon="
                            aiCopiedKey === 'narration' ? 'lucide:check' : 'lucide:clipboard-copy'
                          "
                          width="12"
                        />
                      ...
                      >
                        <Icon
                          :icon="
                            aiCopiedKey === 'narration-inserted'
                              ? 'lucide:check'
                              : 'lucide:plus-circle'
                          "
                          width="12"
                  ...
                  <div class="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      >
                        <Icon
                          :icon="
                            aiCopiedKey === 'concepts' ? 'lucide:check' : 'lucide:clipboard-copy'
                          "
                          width="12"
                        />
